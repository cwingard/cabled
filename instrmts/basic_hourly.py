...
class _Recv(Thread):
    def __init__(self, conn, basename):
        ...
        self._conn = conn
        self.myFileHandler = Logger(basename, 'hourly')
        print "logger initialized with basename %s, will create a new file and rename hourly" % (basename)
        self._last_line = ''
        self._new_line = ''
        self.setDaemon(True)

    # The _update_lines method adds each new character received to the current
    # line or saves the current line and creates a new line
    ...
    # The run method receives incoming chars and sends them to _update_lines,
    # prints them to the console and sends them to the logger.
    def run(self):
        print "### _Recv running."
        while True:
            recv = self._conn.recv(1)
            newline = self._update_lines(recv)
            os.write(sys.stdout.fileno(), recv)  # this writes char by char
            ...
            if newline:
                ...
                self.myFileHandler.write(self._last_line)  # writes to logger file   + "\n"
