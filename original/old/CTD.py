...
This program allows direct user interaction with an CTD instrument via a socket.


USAGE:
    python CTD.py address port basename ### connect to instrument on address:port, with logger basename
    python CTD.py address port ### connect to instrument on address:port, with logger defaulted to generic basename
    python CTD.py port ### connect to instrument on localhost:port, with logger defaulted to generic basename
    
    

Example:
    python CTD.py 10.180.80.169 2101 CTD.180.80.169_2101
...
Commands accepted: 
    "init" - reconfigures instrument to desired configuration. 
...
class _Direct(object):
    ...
    def __init__(self, host, port, basename):
        ...
        # set CTD internal clock
        self.send('DateTime='+time.strftime("%m%d%Y%H%M%S",time.gmtime())+'\r\n') 
        time.sleep(4)
